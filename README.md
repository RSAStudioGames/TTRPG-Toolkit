...
├── Makefile / scripts/  # make dev — UI HMR preview (local only)
...
Open **[http://localhost:8080](http://localhost:8080)** (or the port set in `.env`).

### UI HMR preview (local only)

```bash
make dev
```

Rebuilds the Go API every run, starts it with Vite in one terminal, and hot-reloads frontend changes. Open the Vite URL (usually `http://localhost:5173`), not `:8080`. Ctrl+C stops both.
...
Open **[http://localhost](http://localhost)** — Caddy proxies to the Fiber container on port `8080`. The Go app is not published directly to the host.
